...
public class RequestGETUserPlaylist {
	...
	@Test(priority = 1)
	@Parameters({"BasePathForSelfPlaylist"})
	public void getUserPlaylist(String BasePathForSelfPlaylist)
	{
		RestAssured.given(reqspec).
					when().
					get(BasePathForSelfPlaylist).
					then().
					spec(respspec);
