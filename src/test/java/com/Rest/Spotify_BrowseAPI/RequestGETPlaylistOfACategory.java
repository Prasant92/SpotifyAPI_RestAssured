...
public class RequestGETPlaylistOfACategory {
	...
	@Test(priority = 2)
	@Parameters({"BasePathForGettingAllCategories","CategoryID"})
	public void getPlaylistForASelectedCategory(String BasePathForGettingAllCategories,String CategoryID)
	{
		System.out.println("Started");
		for(int i=0;i<al.size();i++)
		{
			System.out.println(BasePathForGettingAllCategories+"/"+al.get(i)+"/playlists");
			if(al.get(i).equals(CategoryID))
			{
				Response resp=RestAssured.given(reqspec).
						  when().
						  get(BasePathForGettingAllCategories+"/"+al.get(i)+"/playlists").
						  then().
						  spec(respspec).
						  extract().response();
